...
def generateWalkofSigmaDifficulty(meanSigma=15, velocity_std=1, duration=20, minSigma=5, maxSigma=35):
    ...
    increments = np.array([-1.25,-1, -0.75, -0.5, -0.25, 0,0.25, 0.5, 0.75, 1,1.25])
...
# draw text loading
loadingText=visual.TextStim(win, text='Loading...', color=[1, 0, 0], units='pix', height=20)
loadingText.draw()
win.flip()
...
blobs=[]
allPossibleSigma=np.arange(minBlobExp, maxBlobExp+0.50, 0.25)
...
trainingN=1
while trialNum < len(conditions)+trainingN and not endExpNow:
    if trialNum<trainingN:
        ...
        maxSgima= minBlobWidth+(maxBlobExp-minBlobExp)/2
    ...
    while continueRoutine:
        blob_obj = blobVisualObjsDict[sigmaDynamic[realFrameN]]
...
if eyeResp:
    ...
    sio.savemat(filename+'.mat', {'mouse_x': all_mouse_x, 'mouse_y': all_mouse_y, 'blob_x': all_stim_x, 'blob_y': all_stim_y, 'sigma': sigma_trials,'dynamicSigma':allSigmaStim})
elif combinedResp:
